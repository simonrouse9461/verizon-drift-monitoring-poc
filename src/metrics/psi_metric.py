...
class PSIMetric(BaseMetric):
    ...
    def transform_metric(self, source_data: dict[str, DataFrame], spark: SparkSession) -> dict[str, DataFrame]:
        baseline_score = spark.sparkContext.broadcast(
            source_data["baseline"].select("score").toPandas().to_numpy().flatten()
        )
        ...
        realtime_psi = source_data["realtime"].groupby("timestamp").agg(agg_expr)
        realtime_psi.foreach()
